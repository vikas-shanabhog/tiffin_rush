...
export default function RestaurantReviews() {
  ...
  const load = () => {
    ...
    api.get(`/reviews/restaurant/${id}`).then(({ data }) => {
      setReviews(data.reviews);
      setStats({ average: data.average, count: data.count });
    });
    if (user?.role === "customer") {
      api.get(`/reviews/restaurant/${id}/mine`).then(({ data }) => {
        setMine(data);
  ...
  const submit = async (e) => {
    ...
    setSaved(false);
    try {
      await api.post(`/reviews/restaurant/${id}`, form);
      setSaved(true);
      load();
    } catch (err) {
  ...
  if (!restaurant) return <p className="max-w-2xl mx-auto px-5 py-12 text-ink/50">Loading…</p>;

  return (
    <div className="max-w-2xl mx-auto px-5 py-12">
      <Link to={`/restaurants/${id}`} className="text-xs font-mono text-ink/40 hover:text-chili">
        ← Back to menu
      </Link>
      <h1 className="text-2xl font-bold mt-2">{restaurant.name} · Ratings</h1>
      ...
      {!user && (
        <p className="mt-8 text-sm text-ink/50">
          ...
          as a customer to rate this restaurant.
        </p>
      )}
        ...
        (mine.eligible ? (
          <form onSubmit={submit} className="mt-8 p-5 border border-line rounded-xl bg-white space-y-3">
            <h2 className="font-semibold text-sm">
              {mine.review ? "Update your rating" : "Rate this restaurant"}
            </h2>
          ...
          <p className="mt-8 text-sm text-ink/50">
            You can rate this restaurant once an order from here has been delivered to you.
          </p>
        ))}
